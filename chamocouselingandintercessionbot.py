...
async def get_forum_topics(message):
    ...
    # Connect the client if it’s not already connected
        ...
        if not await client.is_user_authorized():
            await client.send_code_request('your phone no')
            await client.sign_in('your phone no', input('Enter the code: '))
        ...
        result = await client(functions.channels.GetForumTopicsRequest(
            ...
            offset_id=0,
            offset_topic=0,
            limit=10  # Specify how many topics you want to retrieve
        ))
        print(result)

        topics_list = [f"Topic ID: {topic.id}, Topic Name: {topic.title}" for topic in result.topics]
        await bot.reply_to(message, "\n".join(topics_list) if topics_list else "No topics found.")

    await fetch_topics()

@bot.message_handler(commands=['view_topics'])
def handle_view_topics(message):
    asyncio.run(get_forum_topics(message))
